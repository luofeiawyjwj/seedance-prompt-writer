...
OpenAI Codex 当前文档推荐的 skill 路径是全局 `$HOME/.agents/skills`，项目内 `.agents/skills`。旧版本或个人环境如果仍识别 `~/.codex/skills`，可以继续使用旧路径，但优先按下面的新路径安装。
...
安装脚本只会写入 `SKILL.md`、`agents/` 和 `references/`，不会把 `.git`、README、examples、evals、scripts 等开源维护文件带进 skill 目录。
